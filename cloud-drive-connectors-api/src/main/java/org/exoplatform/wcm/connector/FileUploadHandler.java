 ...
 * ngoc.tran@exoplatform.com Sep 4, 2009
 */
public class FileUploadHandler {
  ...
  private static final String   CONNECTOR_BUNDLE_LOCATION     = "locale.wcm.resources.WCMResourceBundleConnector";

  private static final String   AUTOVERSION_ERROR_MIME_TYPE   = "DocumentAutoVersion.msg.WrongMimeType";
  ...
  private long                  uploadLifeTime;

  /**
   * Instantiates a new file upload handler.
   */
  public FileUploadHandler() {
    uploadService = ExoContainerContext.getService(UploadService.class);
    ...
    documentService = ExoContainerContext.getService(DocumentService.class);
    repositoryService = ExoContainerContext.getService(RepositoryService.class);
    fckMessage = new FCKMessage();
    uploadIdTimeMap = new Hashtable<>();
    uploadLifeTime = System.getProperty("MULTI_UPLOAD_LIFE_TIME") == null ? 600 : // 10
                                                                                  // minutes
  ...
  /**
   * Check status of uploaded file. If any problem while uploading, error
   * message is returned. Returning null means no problem happen.
   * 
   * @param uploadId upload ID
  ...
  public Response checkStatus(String uploadId, String language) throws Exception {
    ...
    if ((StringUtils.isEmpty(uploadId)) || (uploadService.getUploadResource(uploadId) == null))
      return null;
   ...
   * 'synchronized' is used to avoid JCR index corruption when uploading
   * massively files. The JCR index (that uses a very old version of Apache
   * lucene) gets corrupted when doing massive modifications on the same parent
   * node. Thus here we have made this central method as synchronized to avoid
   * corruption and ensure Data consistency in favor of performances. The upload
   * itself is not synchronized, we still be able to upload concurrently using
   * org.exoplatform.web.handler.UploadHandler but the commit of uploaded file
   * to be stored on JCR is made using this method, thus this critical operation
  ...
  public synchronized Response saveAsNTFile(String workspaceName,
    ...
    try {
      ...
      if (parent.hasNode(fileName)) {
        ...
        // fckMessage.createMessage(FCKMessage.FILE_EXISTED,
        // FCKMessage.ERROR,
        // language,
        // args);
      ...
      AutoVersionService autoVersionService = ExoContainerContext.getService(AutoVersionService.class);
      ...
      if (!CREATE_VERSION.equals(existenceAction) ||
        ...
        if (parent.isNodeType(NodetypeConstant.NT_FILE)) {
          ...
          if (mimetype != mimeTypeParent) {
            ...
            ResourceBundle resourceBundle = resourceBundleService.getResourceBundle(CONNECTOR_BUNDLE_LOCATION,
                                                                                    new Locale(language));
            String errorMsg = resourceBundle.getString(AUTOVERSION_ERROR_MIME_TYPE);
            errorMsg = errorMsg.replace("{0}",
                                        StringUtils.escape("<span style='font-weight:bold;'>" + parent.getName() + "</span>"));
  ...
  private void removeDeadUploads() {
    ...
    for (String id : uploadIdTimeMap.keySet()) {
      if ((System.currentTimeMillis() - uploadIdTimeMap.get(id)) > uploadLifeTime * 1000) {
        removedIds.add(id);
      }
    }
